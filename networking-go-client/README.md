# Go Client For Nutanix Networking APIs

The Go client for Nutanix Networking APIs is designed for Go client application developers offering them simple and flexible access to APIs that manage networking configuration on Nutanix clusters, including AHV and advanced networking.
...
```shell
$ go get github.com/nutanix/ntnx-api-golang-clients/networking-go-client/v4/...@v4.1.1
```
...
require (
	github.com/nutanix/ntnx-api-golang-clients/networking-go-client/v4 latest
)
```
...
## Configuration
The Go client for Nutanix Networking APIs can be configured with the following parameters
...
var (
	ApiClientInstance *client.ApiClient
	AwsSubnetsApiInstance *api.AwsSubnetsApi
)
...
// Initialize the API
AwsSubnetsApiInstance = api.NewAwsSubnetsApi(ApiClientInstance)
page_ := 0
limit_ := 50
...
// 
response, err := AwsSubnetsApiInstance.ListAwsSubnets(&page_, &limit_, &filter_, &orderby_, &select_)
if err != nil {
    ....
}
...
This library has a full set of [API Reference Documentation](https://developers.nutanix.com/sdk-reference?namespace=networking&version=v4.1&language=go). This documentation is auto-generated, and the location may change.
