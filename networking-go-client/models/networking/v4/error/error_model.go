...
func (p *OneOfErrorResponseError) UnmarshalJSON(b []byte) error {
	...
	if err := json.Unmarshal(b, &rawMap); err == nil {
			...
			if valueData, ok := nestedMap["value"]; ok {
				...
				if marshalErr == nil {
					...
					if unmarshalErr == nil {
						...
						if vOneOfType202.ObjectType_ != nil && "networking.v4.error.SchemaValidationError" == *vOneOfType202.ObjectType_ {
							if nil == p.oneOfType202 {
								p.oneOfType202 = new(SchemaValidationError)
							}
		...
		// Check if this field name exists in the map (handles nested structure)
		if nestedMap, ok := rawMap["List<networking.v4.error.AppMessage>"].(map[string]interface{}); ok {
			// Check for "value" wrapper
			if valueData, ok := nestedMap["value"]; ok {
				valueJSON, marshalErr := json.Marshal(valueData)
				if marshalErr == nil {
					...
					if unmarshalErr == nil {
						...
						if vOneOfType201 == nil || len(*vOneOfType201) == 0 || ((*vOneOfType201)[0].ObjectType_ != nil && "networking.v4.error.AppMessage" == *((*vOneOfType201)[0].ObjectType_)) {
	...
	if err := json.Unmarshal(b, vOneOfType201); err == nil {
		if len(*vOneOfType201) == 0 || (vOneOfType201 != nil && (*vOneOfType201)[0].ObjectType_ != nil && "networking.v4.error.AppMessage" == *((*vOneOfType201)[0].ObjectType_)) {
			p.oneOfType201 = *vOneOfType201
			if nil == p.Discriminator {
				p.Discriminator = new(string)
			}
