# Go Client For SP Central Tenant Management

The Go client for SP Central Tenant Management is designed for Go client application developers offering them simple and flexible access to APIs that aPIs for tenant administration, resource allocation and utilization, and tenant user management.
...
```shell
$ go get github.com/nutanix/ntnx-api-golang-clients/tenancy-go-client/v4/...
```
...
require (
	github.com/nutanix/ntnx-api-golang-clients/tenancy-go-client/v4 latest
)
```
...
## Configuration
The Go client for SP Central Tenant Management can be configured with the following parameters
...
import (
	"github.com/nutanix/ntnx-api-golang-clients/tenancy-go-client/v4/client"
	"github.com/nutanix/ntnx-api-golang-clients/tenancy-go-client/v4/api"
)

var (
	ApiClientInstance *client.ApiClient
	TenantsApiInstance *api.TenantsApi
)
...
// ...
domainAllocation := getResponse.GetData().(import1.DomainAllocation)

// The body parameter in the following operation is received from the previous GET request's response which needs to be updated.
response, err := TenantsApiInstance.UpdateTenantDomainAllocationById(&domainAllocation&tenantExtId, &extId, , args)
if err != nil {
....
}
```
...
var (
	ApiClientInstance *client.ApiClient
	MembershipApiInstance *api.MembershipApi
)
...
// Initialize the API
MembershipApiInstance = api.NewMembershipApi(ApiClientInstance)
page_ := 0
limit_ := 50
filter_ := "string_sample_data"

// 
response, err := MembershipApiInstance.ListDomainMemberships(&page_, &limit_, &filter_)
if err != nil {
    ....
}
...
This library has a full set of [API Reference Documentation](https://developers.nutanix.com/sdk-reference?namespace=tenancy&version=v4.0.a1&language=go). This documentation is auto-generated, and the location may change.
