...
/*
  Module dataprotection.v4.common of Nutanix Data Protection APIs
*/
package common
...
import (
	...
	"errors"
	"fmt"
	import2 "github.com/nutanix/ntnx-api-golang-clients/dataprotection-go-client/v4/models/common/v1/config"
...
type ClusterInfo struct {
	...
	ClusterIp *import2.IPAddress `json:"clusterIp,omitempty"`
	/*
...
func (p *ClusterInfo) MarshalJSON() ([]byte, error) {
	// Create Alias to avoid infinite recursion
...
func (p *ClusterInfo) UnmarshalJSON(b []byte) error {
	// Step 1: Unmarshal into a generic map to capture all fields
	var allFields map[string]interface{}
	if err := json.Unmarshal(b, &allFields); err != nil {
		return err
	...
	*p = *NewClusterInfo()
	...
	if known.ClusterIp != nil {
		p.ClusterIp = known.ClusterIp
	}
