...
// This file holds the request struct for the ListHostsByClusterId operation.

type ListHostsByClusterIdRequest struct {
	...
	// filter, aggregate etc. As of now only support for groupby exists.For example '$apply=groupby((templateName))' would get
	// all templates grouped by templateName.
	Apply_ *string
