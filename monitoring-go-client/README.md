# Go Client For Nutanix Monitoring APIs

The Go client for Nutanix Monitoring APIs is designed for Go client application developers offering them simple and flexible access to APIs that manage Alerts, Alert policies, Events and Audits.
...
```shell
$ go get github.com/nutanix/ntnx-api-golang-clients/monitoring-go-client/v4/...
```

##### Install a specific version

```shell
$ go get github.com/nutanix/ntnx-api-golang-clients/monitoring-go-client/v4/...@v4.3.1
```
...
require (
	github.com/nutanix/ntnx-api-golang-clients/monitoring-go-client/v4 latest
)
```
...
## Configuration
The Go client for Nutanix Monitoring APIs can be configured with the following parameters
...
systemDefinedPolicyExtId := "51"
extId := "2d9Ba4cC-2C23-b3ac-fc0D-00fe5B47aD65"

// 
getResponse, err := SystemDefinedPoliciesApiInstance.GetClusterConfigById(&systemDefinedPolicyExtId, &extId)
if err != nil {
....
}
```
...
// ...
clusterConfig := getResponse.GetData().(import1.ClusterConfig)

// The body parameter in the following operation is received from the previous GET request's response which needs to be updated.
response, err := SystemDefinedPoliciesApiInstance.UpdateClusterConfigById(&clusterConfig&systemDefinedPolicyExtId, &extId, , args)
if err != nil {
....
}
```
...
// 
response, err := AlertsApiInstance.ListAlerts(&page_, &limit_, &filter_, &orderby_, &apply_, &expand_, &select_)
if err != nil {
    ....
}
...
This library has a full set of [API Reference Documentation](https://developers.nutanix.com/sdk-reference?namespace=monitoring&version=v4.3&language=go). This documentation is auto-generated, and the location may change.
