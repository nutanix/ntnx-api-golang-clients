...
import (
	...
	"errors"
	"fmt"
	import1 "github.com/nutanix/ntnx-api-golang-clients/monitoring-go-client/v4/models/common/v1/config"
	"time"
)
...
func (p *OneOfErrorResponseError) UnmarshalJSON(b []byte) error {
			...
			if valueData, ok := nestedMap["value"]; ok {
				...
				if marshalErr == nil {
					...
					if unmarshalErr == nil {
						...
						if vOneOfType201 == nil || len(*vOneOfType201) == 0 || ((*vOneOfType201)[0].ObjectType_ != nil && "monitoring.v4.error.AppMessage" == *((*vOneOfType201)[0].ObjectType_)) {
			...
			if valueData, ok := nestedMap["value"]; ok {
				...
				if marshalErr == nil {
					...
					if unmarshalErr == nil {
						...
						if vOneOfType202.ObjectType_ != nil && "monitoring.v4.error.SchemaValidationError" == *vOneOfType202.ObjectType_ {
							if nil == p.oneOfType202 {
								p.oneOfType202 = new(SchemaValidationError)
							}
	...
	if err := json.Unmarshal(b, vOneOfType201); err == nil {
		if len(*vOneOfType201) == 0 || (vOneOfType201 != nil && (*vOneOfType201)[0].ObjectType_ != nil && "monitoring.v4.error.AppMessage" == *((*vOneOfType201)[0].ObjectType_)) {
			p.oneOfType201 = *vOneOfType201
			if nil == p.Discriminator {
				p.Discriminator = new(string)
			}
